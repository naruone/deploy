...
type EnvProServer struct {
    ...
    Uuid           string    `json:"uuid"`
    KeepVersionCnt int       `json:"keep_version_cnt"`
    CreateAt       time.Time `json:"create_at"`
    UpdateAt       time.Time `json:"update_at"`
    Servers        []Server
    Jumper         Server
...
func SaveEnvCfg(env *EnvProServer) (err error) {
    if env.EnvId == 0 {
        ...
        err = mdb.Model(env).Updates(map[string]interface{}{
            ...
            "after_script":     env.AfterScript,
            "keep_version_cnt": env.KeepVersionCnt,
        }).Error
    }
    if err != nil {
        switch {
...
func GetNotKeepTask(envId int, taskId int, keepVersionCnt int) []string {
    var (
        taskList []DeployTask
        res      []string
    )
    mdb.Where("env_id = ?", envId).Where("task_id < ?", taskId).
        Where("status = ?", TaskRunSuccess).Where("uuid != ''").
        Order("task_id desc").Find(&taskList)

    var count = 0
    for _, r := range taskList {
        count++
        if count > keepVersionCnt {
            res = append(res, config.GConfig.ServerWorkDir+"/"+r.Uuid)
            mdb.Model(r).Update("uuid", "")
        }
    }
    return res
}
